...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    //MT: VARIABLE DECLARATIONS & DEFINITIONS
    std::string tempString = sql.c_str();                       //MT: TEXT VERSION OF sql
    std::string newsql;                                         //MT: SANITIZED SQL STRING
    std::smatch match;                                          //MT: PATTERN MATCH WITHIN STRING
    std::regex numInjection(" or [1-9]+=[1-9]+;");              //MT: PATTERN TO MATCH FOR 'OR 1=1;'
    std::regex wordInjection(" or '[a-zA-Z]+'='[a-zA-Z]+';");   //MT: PATTERN TO MATCH FOR 'OR 'he'='he';'

    //MT: SEARCH FOR / DETECT SQL INJECTION PATTERN MATCH - NUMERIC
    //MT: IF DETECTED, REPLACE THE MATCHING PATTERN WITH A SEMICOLON ';'
    //MT: RERUN THE QUERY USING THE SANITIZED SQL STRING 'newsql'
      ...
      //MT: SEARCH FOR / DETECT SQL INJECTION PATTERN MATCH - ALPHA
